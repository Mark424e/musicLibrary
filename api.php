...
if (isset($data["password"]) && $data["password"] === "Qwerty123") {
    ...
    if (!empty($data["nameSearch"])) {
        $sql .= " AND (songName LIKE CONCAT('%', :songName, '%') OR songArtist LIKE CONCAT('%', :songName, '%') OR songDescShort LIKE CONCAT('%', :songName, '%')) ";
        $bind[":songName"] = $data["nameSearch"];
    }
    $sql .= "ORDER BY songName";

    $songs = $db->sql($sql, $bind);
